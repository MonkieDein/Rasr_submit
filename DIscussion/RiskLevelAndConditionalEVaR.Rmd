...
Define problem
```{r}
X1 = data.frame(x = c(1,10),p = c(0.3,0.7))
X2 = data.frame(x = c(2,10),p = c(0.8,0.2))
condp = c(0.2,0.8)
Xjoint = data.frame(x = c(1,2,10),p =c(0.06,0.64,0.30))
```
...
```{r}
Xi1 = 0.118/alpha#((1:999)*0.001)/alpha
A1 = 1-(1-alpha)*Xi1
# A1 = 0.6482006009142005087398
# Xi1 = (1-A1)/(1-alpha)
Xi2 = (1-condp[1]*Xi1)/condp[2]
A2 = 1-(1-alpha)*Xi2
filter = which(((condp[1]*Xi1*log(Xi1) + condp[2]*Xi2*log(Xi2) ) <= -log(1-alpha))&((1-alpha)*Xi1 <= 1)&((1-alpha)*Xi2 <= 1)&(A1>0.7))
```

Find EVaR
```{r}
E(X = c(Xi1*1,Xi2*2),prob=condp)
```
...
alp1 = 1-exp(-E(exp(Z1)*logZ1,X1$p))
EVaR1= E(Z1*X1$x,prob=X1$p)
```
