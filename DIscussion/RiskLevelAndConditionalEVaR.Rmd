...
\[
\text{EVaR}_\alpha[X] \neq \inf \{~ \mathbb{E}[\xi_\tau \cdot \text{EVaR}_{\alpha \xi_T}[X | \mathcal{F}_\tau] ] ~ : \mathbb{E}[\xi_\tau] = 1 ,\mathbb{E}[\xi_\tau \log( \xi_\tau )] \leq -\log(1-\alpha)\} 
\]
...
condp = c(0.2,0.8)
Xjoint = data.frame(x = c(1,2,10),p =c(0.04,0.72,0.24))
```
...
\[
\inf \{~ \mathbb{E}[\xi_\tau \cdot \text{EVaR}_{\alpha \xi_T}[X | \mathcal{F}_\tau] ] ~ : \mathbb{E}[\xi_\tau] = 1 ,\mathbb{E}[\xi_\tau \log( \xi_\tau )] \leq -\log(1-\alpha)\} 
\]
...
```{r}
A1 = 0.5866321807770045
Xi1 = A1/alpha
Xi2 = (1-condp[1]*Xi1)/condp[2]
A2 = Xi2 * alpha
(condp[1]*Xi1*log(Xi1) + condp[2]*Xi2*log(Xi2) ) <= -log(1-alpha) 
```

Find EVaR alpha for X1
```{r}
b1 = 5.386218547069895
ERMb1 = ERM(X1$x,alpha=b1,prob=X1$p)
EVaR1 = ERMb1 + log(1-A1)/b1 
logZ1 = -b1*(X1$x - ERM(X1$x,alpha=b1,prob=X1$p))
EVaR_ub1 = sum(X1$p*exp(logZ1)*X1$x)
E(X = c(Xi1*EVaR_ub1,Xi2*2),prob=condp)
E(X = c(Xi1*EVaR1,Xi2*2),prob=condp)
```
